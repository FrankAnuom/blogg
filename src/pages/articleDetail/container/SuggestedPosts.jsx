    ...
    >
      ...
      <div className="grid gap-y-5 mt-5 md:grid-cols-2 md:gap-x-5 lg:grid-cols-2">
        {posts.map((item) => (
          ...
          >
            <img
              className="aspect-square object-contain rounded-lg w-1/5"
              src={item.image}
              alt="ECG"
            />
      ...
      <h2 className="font-roboto text-dark-hard mt-8 md:text-xl ">Tags</h2>
      <div className="flex flex-wrap gap-x-2 gap-y-2 mt-4">
        {tags.map((item, index) => (
          <Link
            key={index}
            to="/"
            className="inline-block rounded-md px-3 py-1.5 bg-primary font-roboto text-white md:text-sm"
          >
            {item}
          </Link>
        ))}
